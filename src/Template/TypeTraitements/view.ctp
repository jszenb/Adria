...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
	...
	<ul class="side-nav">
		<li><?= $this->Html->link(__('Modifier ce type de traitement'), ['action' => 'edit', $typeTraitement->id]) ?></li>
    </ul>
</nav>
<div class="typeTraitements view large-9 medium-8 columns content">
    <h3>Type de traitement : <?= h($typeTraitement->type) ?></h3>
    ...
    <div class="row">
        ...
        <?= $this->Text->autoParagraph(h($typeTraitement->description)); ?>
    </div>
	<h4><?= __('Fonds concerné(s)') ?></h4>
    <div class="related">
        <?php if (!empty($typeTraitement->fonds)): ?>
        <table cellpadding="0" cellspacing="0">
            ...
            <?php foreach ($typeTraitement->fonds as $fonds): ?>
            <tr>
                <td><?= h($fonds->nom) ?></td>
				<?php 
					$libelleTypeFonds = "";
