...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
	...
	<ul class="side-nav">
		<li><?= $this->Html->link(__('Modifier cette discipline'), ['action' => 'edit', $thematique->id]) ?></li>
    </ul>
</nav>
<div class="thematiques view large-9 medium-8 columns content">
    <h3>Discipline : <?= h($thematique->intitule) ?></h3>
    ...
    <div class="row">
        ...
        <?= $this->Text->autoParagraph(h($thematique->description)); ?>
    </div>
	<h4><?= __('Fonds concerné(s)') ?></h4>
    <div class="related">
        <?php if (!empty($thematique->fonds)): ?>
        <table cellpadding="0" cellspacing="0">
            ...
            <?php foreach ($thematique->fonds as $fonds): ?>
            <tr>
                <td><?= h($fonds->nom) ?></td>
				<?php 
					$libelleTypeFonds = "";
