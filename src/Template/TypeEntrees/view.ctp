...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
	...
	<ul class="side-nav">
		<li><?= $this->Html->link(__('Modifier ce type de mode d\'entrée'), ['action' => 'edit', $typeEntree->id]) ?></li>
    </ul>
</nav>
<div class="typeEntrees view large-9 medium-8 columns content">
    <h3>Type de mode d'entrée : <?= h($typeEntree->type) ?></h3>
    ...
    <div class="row">
        ...
        <?= $this->Text->autoParagraph(h($typeEntree->description)); ?>
    </div>
	<h4><?= __('Fonds concerné(s)') ?></h4>
    <div class="related">
        <?php if (!empty($typeEntree->fonds)): ?>
        <table cellpadding="0" cellspacing="0">
            ...
            <?php foreach ($typeEntree->fonds as $fonds): ?>
            <tr>
                <td><?= h($fonds->nom) ?></td>
				<?php 
					$libelleTypeFonds = "";
