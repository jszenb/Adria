...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
	...
	<ul class="side-nav">
		<li><?= $this->Html->link(__('Modifier ce type d\'instrument de recherche'), ['action' => 'edit', $typeInstrRech->id]) ?></li>
    </ul>
</nav>
<div class="typeInstrRechs view large-9 medium-8 columns content">
    <h3>Type d'instrument de recherche : <?= h($typeInstrRech->type) ?></h3>
    ...
    <div class="row">
        ...
        <?= $this->Text->autoParagraph(h($typeInstrRech->description)); ?>
    </div>
	<h4><?= __('Fonds concerné(s)') ?></h4>
    <div class="related">
        <?php if (!empty($typeInstrRech->fonds)): ?>
        <table cellpadding="0" cellspacing="0">
            ...
            <?php foreach ($typeInstrRech->fonds as $fonds): ?>
            <tr>
                <td><?= h($fonds->nom) ?></td>
				<?php 
					$libelleTypeFonds = "";
